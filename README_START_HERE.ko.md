...
공식 문서 기준으로 Docker Quickstart compose 파일은 `docker/docker-compose.quickstart.yml`에 있고, 수동 빌드는 repo 루트 `Dockerfile`을 사용합니다.
...
4. 에이전트가 checkout -> 계획 -> 수정 -> 브라우저 리뷰 -> verify -> 코멘트/아티팩트 업로드 수행
